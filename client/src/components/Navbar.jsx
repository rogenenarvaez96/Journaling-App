...
import { Book, Image as ImageIcon, BarChart2, Archive, LogOut, Shield, Sun, Moon, Settings } from 'lucide-react';
  ...
  return (
    <nav className="w-full bg-stone dark:bg-obsidian border-b border-black/5 dark:border-white/5 px-6 py-4 sticky top-0 z-50 flex items-center justify-between transition-colors duration-500 text-obsidian dark:text-stone">
      ...
      <div className="flex items-center gap-4">
        ...
        <button onClick={handleLogout} className="p-2 bg-black/5 dark:bg-white/5 rounded-full hover:bg-terracotta hover:text-white transition-colors" title="Log Out">
          <LogOut size={16} />
        </button>
      </div>
    </nav>
